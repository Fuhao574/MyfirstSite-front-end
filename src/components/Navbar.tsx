...
export default function Navbar() {
  ...
  const [navToasts, setNavToasts] = useState<{ id: number; msg: string; type: ToastType }[]>([]);
  const navToastIdRef = useRef(0);
  const avatarAreaRef = useRef<HTMLDivElement>(null);

  // Navbar 级 Toast（ProfileCenter 关闭后继续显示）
  const showNavToast = (msg: string, type: ToastType = 'info') => {
        ...
        <NotificationContainer>
          {navToasts.map((t) => (
            <NotificationItem
              ...
              message={t.msg}
              duration={3000}
              onClose={() => setNavToasts((prev) => prev.filter((item) => item.id !== t.id))}
            />
          ))}
        </NotificationContainer>
      )}
